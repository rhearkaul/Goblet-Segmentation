...
def save_annotations(points, image_path, boxes=[]):
    ...
    np.savez(
        "watershed.npz", points=points_array, boxes=boxes_array, image_path=image_path
    )
...
def watershed_image(bin_image_path, image_path):
    ...
    coords, _, _, _ = watershed.generate_centroid(
        image=rgb_image, bin_mask=bin_image_bool.astype(bool), **kwargs
    )
